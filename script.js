...
async function main() {
  ...
  let heartImages = await getHeartImages();
  console.log("Danh sách hình ảnh trái tim:", heartImages);
  ...
  for (let group = 0; group < numGroups; group++) {
    ...
    groupGeometryNear.setAttribute(
      ...
      new THREE.BufferAttribute(
        groupColorsNear.slice(0, validPointCount * 3),
        3
      )
    );
  ...
  function animatePlanetSystem() {
    if (window.textRings) {
      ...
      window.textRings.forEach((ringGroup, index) => {
        ...
        const verticalBob =
          Math.sin(time * (userData.tiltSpeed * 0.7) + userData.tiltPhase) *
          0.3;
  ...
  function animate() {
    ...
    scene.traverse((obj) => {
      if (
        obj.isPoints &&
        obj.userData.materialNear &&
        obj.userData.materialFar
      ) {
